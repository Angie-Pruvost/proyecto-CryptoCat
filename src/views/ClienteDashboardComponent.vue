<template>
  ...
  <div>
    <button @click="$router.push(`/historico/${p.crypto}`)">Ver histórico</button>
  </div>
    <!-- ==================== CARTERA ACTUAL ==================== -->
    <section class="card">
...
.btn.edit { background: #2764a4; color: rgb(255, 255, 255); }
.btn.sell { background: #017949; color: white; }
.btn.delete { background: #a42f2f; color: white; }
.btn.save { background: #237cdb; color: white; }
.btn.cancel { background: #814c4c; color: white; }
.green { color: green; }
.red { color: red; }
...
.tabla-portfolio tr:hover {
  background: rgba(139, 55, 55, 0.77);
  transition: 0.2s;
}
.btn.sell:hover {
